...
function validate(){
    ...
    //3)주민번호 앞칸이 숫자형 글자인지, 글자갯수가 6인지 확인하시오
    let jumin1=document.getElementById("jumin1").value;
    jumin1=jumin1.trim(); 
    if(jumin1.length!=6 || isNaN(jumin1)){ //숫자기호인지?
        alert("주민번호(앞칸) 6글자 이내로 입력해주세요~");
        document.getElementById("jumin1").value=""; //기존값을 지우기 위해 반문자열 대입
